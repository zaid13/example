...
void main() {
  runApp(const MaterialApp(home: MyApp()));
    ...
    // Create a logger for the 'flutter_blue_elves' library
    final logger = Logger('flutter_blue_elves');
...
class _MyAppState extends State<MyApp> {
  ...
  void initState() {
    ...
    suppressFlutterBlueElvesLogs();
    super.initState();
    // Timer.periodic(const Duration(milliseconds: 2000),2000
  ...
  void suppressFlutterBlueElvesLogs() {
    // Create a logger for the 'flutter_blue_elves' library
    void suppressFlutterBlueElvesLogs() {
      // Enable hierarchical logging
      Logger.root.level = Level.ALL;
...
Future.delayed(Duration(seconds: 1),() async {
        ...
        itemBuilder: (BuildContext context, int index) {
          if (_connectedList.isNotEmpty && index <= _connectedList.length) {
            ...
            if (index == scanStartIndex) {
              ...
              // if(currentScan.name=='DESKTOP-G42VE0Q'){
  ...
  Start_Scanning(){
    FlutterBlueElves.instance.startScan(5000).listen((ScanResult event) {

      if(event.name!=null && event.name!.contains('DESKTOP-G42V')){

        FlutterBlueElves.instance.stopScan();

       Device device=  event.connect(connectTimeout: 10000);
